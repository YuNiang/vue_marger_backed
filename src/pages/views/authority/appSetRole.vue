...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      // 当前用户对应角色信息----- 编辑时有值
      API.GetAuthAppRole({ appID: this.form.appID })
        .then(result => {
          let {success, errMsg, data} = result;
          if (!success) {
            this.$message.error(errMsg || '错误！');
          ...
          if (data) {
            let {appRoleArr, roleRouteArr} = data;
            this.form.roleIds = [];
            appRoleArr.forEach(appRole => {
              this.form.roleIds.push(appRole.roleID)
            });
            this.setData(appRoleArr, roleRouteArr);
          }
        });
    },
    ...
    methods: {
      ...
      handleClick () {
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.noClick = true;
            let {deleteIds, addAppRoles, noChangeAppRoles} = this.getParams();
            API.POSTAuthAppRoleBulk({deleteIds, addAppRoles})
              .then(result => {
                let {success, errMsg, data} = result;
                ...
                if (!success) {
                  ...
                  this.setData(data.concat(noChangeAppRoles), this.routes.roleRouteArr);
      ...
      getParams () {
        let noChangeAppRoles = [], deleteIds = [], addAppRoles = [],
          newRoleIds = this.form.roleIds, oldRoleIds = this.default.roleIds, oldAppRole = this.default.appRoleArr;
        oldAppRole.forEach(appRole => {
          if (!newRoleIds.includes(appRole.roleID)) {
            deleteIds.push(appRole.id)
          } else {
            noChangeAppRoles.push(appRole)
          }
        });
